...
- **`AppModel`** — `@MainActor ObservableObject`, the central coordinator:
  ...
  - `setShelfOrder(_:)` directly persists a new shelf item order from the drag-and-drop visual state, replacing error-prone index arithmetic
