...
6. **操作** — `DefaultMenuBarInteractionRouter` が AXPress 対応項目はまず Accessibility 経由で直接クリックを試行（overlay を隠さず実行）、成功すれば即座に折りたたみ；非対応または失敗時は overlay を先に隠してから遅延して CGEvent マウスイベントを合成し、クリック後に自動的に状態を更新；棚は左クリックによるアクティベートと右クリックによるコンテキストメニューの両方をサポート
