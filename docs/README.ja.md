...
- **リキッドグラス UI** — フロストマスク ＋ 縦型バブルカード棚 ＋ スプリングアニメーション。Apple 最新のデザイン言語にインスパイア
...
6. **操作** — `DefaultMenuBarInteractionRouter` が AXPress 対応項目は Accessibility 経由で直接クリック、非対応項目は CGEvent マウスイベントを合成；棚は左クリックによるアクティベートと右クリックによるコンテキストメニューの両方をサポート
